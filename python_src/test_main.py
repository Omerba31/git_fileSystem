...
def test_save_content():
    ...
    file_manager.save_content(OBJECTS_DIR, test_filepath)
    hash_output = file_manager.compute_sha1(test_filepath)
...
def test_open_content():
    ...
    assert fd != -1, "Failed to open the content"
...
def test_save_and_delete_content():
    ...
    assert os.path.exists(saved_file_path), "Content was not saved correctly"
    file_manager.delete_content(OBJECTS_DIR, hash_output)
...
def test_open_non_existent_file():
    ...
    try:
        file_manager.open_content(OBJECTS_DIR, non_existent_hash)
    except OSError as e:
...
def test_delete_non_existent_file():
    non_existent_hash = "deadbeef" + "0" * 32
    try:
        file_manager.delete_content(OBJECTS_DIR, non_existent_hash)
...
def test_lock_blocking():
    ...
    def save_content():
        try:
            fd = file_manager.open_content(OBJECTS_DIR, hash_output)
            assert fd != -1, "Failed to open file in save_content"
            save_done.set()
            time.sleep(3)
            os.close(fd)
        except Exception as e:
    ...
    def open_locked_file():
        assert save_done.wait(timeout=5), "Timeout waiting for save_content to lock the file."
        fd = file_manager.open_content(OBJECTS_DIR, hash_output)
    ...
    def save_content():
        print("save_content started.")
        try:
            fd = file_manager.open_content(OBJECTS_DIR, hash_output)
            assert fd != -1, "Failed to open file in save_content"
            print(f"save_content: acquired lock with fd={fd}.")
            ...
            print("save_content: released lock.")
        except Exception as e:
            print(f"save_content error: {e}")
            save_done.set()

    def open_locked_file():
        print("open_locked_file: Waiting for save_content to signal lock.")
        if not save_done.wait(timeout=5):
            raise TimeoutError("Timeout waiting for save_content to lock the file.")
...
def test_multi_file_lock_contention():
    ...
    def save_content(filepath):
        file_manager.save_content(OBJECTS_DIR, filepath)
